...
The project includes a Streamlit chatbot interface powered by OpenAI/OpenRouter:

### Setup

1. Set your API key (optional - app works in template mode without it):
```bash
...
**Note**: If you see a warning about "LLM Features Unavailable", the app is running in **template-based mode**. This is normal and the core functionality (drug interaction checking) still works perfectly. See [TROUBLESHOOTING.md](TROUBLESHOOTING.md) for details.
...
- **AI-Powered Responses** (when available): Intelligent responses and explanations using OpenAI/OpenRouter
- **Template-Based Mode**: Works without AI - provides template-based explanations when LLM is unavailable
...
If you encounter issues, especially related to PyTorch DLL errors on Windows, see [TROUBLESHOOTING.md](TROUBLESHOOTING.md) for detailed solutions.

Common issues:
- **PyTorch DLL Error**: Application runs in template mode (core features still work)
- **API Key Not Found**: Application works without API key in template mode
