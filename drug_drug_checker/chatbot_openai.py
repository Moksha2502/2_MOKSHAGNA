...
# Lazy import to avoid PyTorch DLL errors on Windows
...
def _lazy_import_langchain():
    """Lazy import of LangChain modules to avoid DLL initialization errors."""
    global OPENAI_AVAILABLE, ChatOpenAI, ChatPromptTemplate, HumanMessage, SystemMessage
    
    if OPENAI_AVAILABLE is not False or ChatOpenAI is not None:
    ...
    # If DLL error occurred, don't try fallbacks
    ...
    if not OPENAI_AVAILABLE:
        try:
            ...
            # Create simple message classes for compatibility
            class SimpleMessage:
                def __init__(self, content):
                    self.content = content
            HumanMessage = SystemMessage = SimpleMessage
            OPENAI_AVAILABLE = True
...
class DrugInteractionChatbot:
    ...
    def __init__(self, api_key: Optional[str] = None, data_path: Optional[str] = None, 
        ...
        """
        # Get API key (optional - app works without it in template mode)
        if api_key:
        ...
        # API key is optional - app will work in template mode without it
        # if not self.api_key:
        ...
        if OPENAI_AVAILABLE and ChatOpenAI:
            try:
                ...
                if "401" in error_str or "invalid" in error_str or "api key" in error_str or "unauthorized" in error_str:
                    ...
                    print("The application will work in template-based mode without AI-generated responses.")
                else:
            ...
            print("Warning: langchain-openai not available (possibly due to PyTorch DLL error).")
            print("The application will work in template-based mode without AI-generated responses.")
            print("To enable AI features, install Visual C++ Redistributables and reinstall PyTorch.")
            self.llm_available = False
        ...
        # Update RAG system to use API (embeddings already initialized correctly based on use_openrouter flag)
        if self.checker.rag_system:
            ...
            # Only re-initialize LLM, embeddings are already set correctly (local embeddings for OpenRouter)
...
Be direct and informative. Start your response immediately with the findings."""
        ...
        # Generate response using OpenAI/OpenRouter or template-based fallback
        if self.llm_available and self.llm:
            try:
                print(f"[DEBUG] LLM is available, making API call to OpenRouter...")
                # Lazy import to ensure modules are loaded
                _lazy_import_langchain()
                ...
                print(f"[DEBUG] Calling OpenRouter API with model: {self.model_name}")
                response = self.llm.invoke(messages)
                result = response.content if hasattr(response, 'content') else str(response)
                print(f"[DEBUG] LLM response received (length: {len(result)} chars)")
                ...
                print(f"[DEBUG] llm_available={self.llm_available}, llm={self.llm is not None}")
        else:
            # Template-based response when LLM is not available
            print(f"[DEBUG] Using template mode - llm_available={self.llm_available}, llm={self.llm is not None}")
        
        # Template-based fallback response
        if interaction_result and interaction_result.get('count', 0) > 0:
            response = "## Drug Interaction Analysis\n\n"
            response += self.format_interactions_for_llm(interaction_result)
            response += "\n\n⚠️ **Important:** Always consult with a healthcare professional before making any medication decisions."
            if not self.llm_available:
                response += "\n\n*Note: Running in template-based mode. AI-generated explanations are not available due to PyTorch DLL error.*"
            return response
        elif medications and len(medications) >= 2:
            response = f"## Interaction Check Results\n\n"
            response += f"No known interactions found between {', '.join(medications)}.\n\n"
            response += "⚠️ **Note:** This does not guarantee safety. Always consult with a healthcare professional."
            if not self.llm_available:
                response += "\n\n*Note: Running in template-based mode. AI-generated explanations are not available due to PyTorch DLL error.*"
            return response
        else:
            response = "I can help you check for drug-drug interactions. Please provide at least two medication names.\n\n"
            response += "For example: 'Check interactions between Aspirin and Warfarin'"
            if not self.llm_available:
                response += "\n\n*Note: Running in template-based mode. AI-generated explanations are not available due to PyTorch DLL error.*"
            return response
